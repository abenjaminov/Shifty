...
const genetic_models_1 = require("./genetic.models");
const models_1 = require("../models/models");
const linq_1 = __importDefault(require("linq"));
class NormalizedFitness {
    constructor(value, chromosomeIndex) {
        this.value = value;
...
class GeneticEnviroment {
    ...
    start(context) {
        var selectProfiles = context.select(models_1.Profile, true);
        var selectRooms = context.select(models_1.Room, true);
        var selectConditions = context.select(models_1.Condition, true);
    ...
    run(profiles, rooms) {
        try {
            ...
            var bestPopulation = new genetic_models_1.Population(0);
            while (population.fitness < maxPopulationFitness) {
                var newPopulation = new genetic_models_1.Population(population.generation + 1);
            ...
            var bestChromosome = linq_1.default.from(population.chromosomes).orderByDescending((x) => x.fitness).first();
            bestChromosome.genes = linq_1.default.from(bestChromosome.genes).where(g => !g.isEmpty).toArray();
        }
        catch (error) {
            console.log(error);
        }
    ...
    elitism(oldPopulation, newPopulation, elitismRate) {
        var elitePopulation = new genetic_models_1.Population(newPopulation.generation);
        var nOfElites = Math.round(oldPopulation.chromosomes.length * elitismRate);
        var nOfChromosomes = newPopulation.chromosomes.length;
        var oldElites = linq_1.default.from(oldPopulation.chromosomes).orderByDescending((x) => x.fitness).take(nOfElites);
        var bestOfNew = linq_1.default.from(newPopulation.chromosomes).orderByDescending((x) => x.fitness).take(nOfChromosomes - nOfElites);
        elitePopulation.chromosomes.push(...oldElites);
        elitePopulation.chromosomes.push(...bestOfNew);
        return elitePopulation;
    }
    mutation(chromosome, mutationProbability) {
        var mutatedGenes = [];
        var notGoodOrNiceToHaveGenes = linq_1.default.from(chromosome.genes).where((g) => !g.isGood());
    ...
    crossover(couple, crossoverProbability) {
        ...
        var chromosome1Enumerable = linq_1.default.from(couple.chromosome1.genes);
        var chromosome2Enumerable = linq_1.default.from(couple.chromosome2.genes);
        ...
        var child1 = new genetic_models_1.Chromosome();
        child1.genes.push(...parent1FirstPart);
        child1.genes.push(...parent2SecondPart);
        child1.genes.push(...parent1ThirdPart);
        var child2 = new genetic_models_1.Chromosome();
        child2.genes.push(...parent2FirstPart);
        child2.genes.push(...parent1SecondPart);
        child2.genes.push(...parent2ThirdPart);
        var doubleChild1Indexes = linq_1.default.from(child1.genes).select((g, i) => {
            return { gene: g, index: i };
        }).groupBy((x) => x.gene.profile.id).where((x) => x.count() > 1).
            selectMany((x) => x.toArray().map(a => a.index)).toArray();
        var doubleChild2Indexes = linq_1.default.from(child2.genes).select((g, i) => {
            return { gene: g, index: i };
        }).groupBy((x) => x.gene.profile.id).where((x) => x.count() > 1).
            selectMany((x) => x.toArray().map(a => a.index)).toArray();
    ...
    selection(population) {
        var sortedChromosomes = linq_1.default.from(population.chromosomes).orderBy((x) => x.fitness).distinct((c) => c.fitness).toArray();
        if (sortedChromosomes.length == 1) {
            return undefined;
        }
        var fitnessSum = linq_1.default.from(sortedChromosomes).sum((c) => c.fitness);
        var normalizedFitnesses = linq_1.default.from(sortedChromosomes).select((c, index) => { return new NormalizedFitness(c.fitness / fitnessSum, index); }).
    ...
    getChromosomeIndexForSelection(normalizedFitnesses, cumilatedSum) {
    ...
    generatePopulation(profiles, rooms, numberOfChromosomes) {
        var result = new genetic_models_1.Population(0);
        ...
        for (let i = 0; i < missingProfiles; i++) {
            emptyProfiles.push(models_1.Profile.Empty(i + 1));
        }
        for (let i = 0; i < numberOfChromosomes; i++) {
            let chromosome = new genetic_models_1.Chromosome();
            let randomProfiles = linq_1.default.from(profiles).orderBy((p) => Math.random()).toArray();
            for (let j = 0; j < geneTemplates.length; j++) {
                let newGene = new genetic_models_1.Gene(geneTemplates[j].roomId, geneTemplates[j].profession, geneTemplates[j].profile, geneTemplates[j].importance);
    ...
    getGeneTemplates(rooms, numOfProfiles) {
        var geneTemplates = [];
        for (let room of rooms) {
            for (let condition of room.conditions) {
                for (let index = 0; index < condition.amount; index++) {
                    let geneTemplate = new genetic_models_1.Gene(room.id, condition.profession, models_1.Profile.Empty(-1), condition.importance);
        ...
        if (geneTemplates.length < numOfProfiles) {
            ...
            for (let index = 0; index < diff; index++) {
                let emptyGene = genetic_models_1.Gene.Empty(index);
                geneTemplates.push(emptyGene);
            }
        }
        return geneTemplates;
    }
    calculateMaximumAvailableFitness(profiles, rooms) {
        var maxFitness = 0;
        var countByTags = linq_1.default.from(rooms).selectMany((r) => r.conditions).
            groupBy((x) => { return { tagId: x.profession.id, importance: x.importance }; }, (x) => x, (key, elements) => {
            return linq_1.default.from(elements).where((c) => c.profession.id == key.tagId && c.importance == key.importance).toArray();
        }).toDictionary((x) => { return { tag: x[0].profession, importance: x[0].importance }; }, (c) => c.length);
        for (let valuePair of countByTags.toEnumerable()) {
            if (valuePair.key.importance == models_1.ConditionImportance.Required) {
                maxFitness += 2 * valuePair.value;
            }
            else if (valuePair.key.importance == models_1.ConditionImportance.NiceToHave) {
                maxFitness += 1 * valuePair.value;
            }
        }
        var usedProfiles = [];
        var requiredConditions = countByTags.toEnumerable().where((x) => x.key.importance == models_1.ConditionImportance.Required);
        var niceToHaveConditions = countByTags.toEnumerable().where((x) => x.key.importance == models_1.ConditionImportance.NiceToHave);
        var profilesEnumerable = linq_1.default.from(profiles);
        for (let reqCondition of requiredConditions) {
            ...
            if (difference > 0) {
                if (reqCondition.key.importance == models_1.ConditionImportance.Required) {
                    maxFitness -= 2 * difference;
                }
                else if (reqCondition.key.importance == models_1.ConditionImportance.NiceToHave) {
...
exports.GeneticEnviroment = GeneticEnviroment;
//# sourceMappingURL=evniroment.js.map